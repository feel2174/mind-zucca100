...
export function BurnoutQuiz({ title, description }: { title?: string; description?: string }) {
    ...
    const {
        ...
        restartQuiz,
        nextQuestion,
    } = useQuiz({ slug: "/burnout", loadingDuration: 3500 });
    ...
    const handleAnswer = (score: number) => {
        ...
        if (isComplete) {
            completeQuiz(nextScore.toString());
        }
    };
            ...
            <AnimatePresence mode="wait">
                {step === "intro" && (
                    <motion.div key="intro" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
                        <QuizHeader
                            category="번아웃 자가진단"
                            categoryHref="/burnout"
                            title={title || "직장인 번아웃 자가진단"}
                            description={description || "나의 번아웃 지수를 체크해보세요."}
                            badge="MENTAL CARE GUIDE"
                            themeColor="orange"
                            stats={[
                        ...
                        <QuizIntroCard
                            icon="🤯"
                            title="요즘 너무 힘들지 않나요?"
                            description="요즘 따라 기운이 없고 회사 업무가 고통스럽나요? 15가지 질문을 통해 나의 번아웃 지수를 체크해보세요."
                            buttonText="🚀 진단 시작하기"
                            onStart={startQuiz}
                ...
                {step === "quiz" && (
                    <motion.div key="quiz" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-10">
                        <QuizProgress progress={progress} themeColor="orange" />
