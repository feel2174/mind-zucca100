            ...
            <AnimatePresence mode="wait">
                {step === "intro" && (
                    <motion.div
                        ...
                        className="rounded-[2.5rem] bg-white p-8 md:p-12 text-center shadow-xl border border-slate-100"
                    >
                        <div className="mb-8 text-7xl">🤯</div>
                        <h2 className="mb-4 text-4xl font-black text-slate-900 tracking-tight">직장인 번아웃<br />자가진단</h2>
                ...
                {step === "result" && (
                    <motion.div
                        ...
                        className="space-y-8"
                    >
                        <div className={`overflow-hidden rounded-[3rem] bg-white shadow-2xl border-t-8 ${result.borderColor}`}>
                            <div className="p-10 md:p-14 text-center">
                                <div className="text-8xl mb-6 animate-bounce-slow">{result.emoji}</div>
                                <div className={`inline-block rounded-full ${result.bgColor} px-6 py-2 text-sm font-black ${result.textColor} mb-6 tracking-widest uppercase`}>
                                    {result.level}
                                </div>
                                <h2 className="text-4xl font-black text-slate-900 leading-tight">{result.title}</h2>
                                <p className="mt-6 text-xl text-slate-600 font-medium leading-relaxed">{result.description}</p>
                                ...
                                <div className="mt-12 border-t border-slate-100 pt-10 text-left">
                                    ...
                                    <ul className="mt-6 space-y-4">
                                        {result.advice.map((a, i) => (
                                            <li key={i} className="flex items-start gap-5 text-lg font-bold text-slate-700 bg-slate-50 p-6 rounded-3xl">
